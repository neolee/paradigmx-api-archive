...
(def ^:dynamic *nwords* (train (words (slurp (io/resource "big.txt")))))
...
(defn correct
  ([word nwords] (let [candidates (or (known [word] nwords) (known (edits1 word) nwords)
                                      (known-edits2 word nwords) [word])]
                   (apply max-key #(get nwords % 1) candidates)))
  ([word] (correct word *nwords*)))
